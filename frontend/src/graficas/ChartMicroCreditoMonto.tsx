...
export function ChartMicroCreditoMonto() {
  const [info, setinfo] = useState<consulta[]>([]);
  const where = whereCluster();
  const { granularidad } = useGranularidad();

  const queryCluster = `SELECT cast(Cluster as char) label, avg(Total_Monto_Microcredito) valor FROM dataSetMIAD ${where} GROUP BY Cluster ORDER BY Cluster asc`;
  const queryMunicipio = `SELECT Municipio label, Total_Monto_Microcredito valor FROM dataSetMIAD ${where} ORDER BY valor desc`;
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `Monto de Micro Credito por ${granularidad}`,
        font: {
          size: 24,
