...
export function ChartIndicador() {
  ...
  const queryCluster = `SELECT cast(Cluster as char) label, avg(INDICADOR) valor FROM dataSetMIAD ${where} GROUP BY Cluster ORDER BY Cluster asc`;
  const queryMunicipio = `SELECT Municipio label, INDICADOR valor FROM dataSetMIAD ${where} ORDER BY valor desc`;
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `Indicador por ${granularidad}`,
        font: {
          size: 24,
