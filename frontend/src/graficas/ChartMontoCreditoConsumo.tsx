...
export function ChartMontoCreditoConsumo() {
  const [info, setinfo] = useState<consulta[]>([]);
  const where = whereCluster();
  const { granularidad } = useGranularidad();

  const queryCluster = `SELECT cast(Cluster as char) label, sum(Total_Monto_Credito_consumo ) valor FROM dataSetMIAD ${where} GROUP BY Cluster ORDER BY Cluster asc`;
  const queryMunicipio = `SELECT Municipio label, Total_Monto_Credito_consumo valor FROM dataSetMIAD ${where} ORDER BY valor desc`;
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `Monto Credito Consumo por ${granularidad}`,
        font: {
          size: 24,
