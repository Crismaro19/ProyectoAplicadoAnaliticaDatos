...
export function ChartCertDepoTermino() {
  const [info, setinfo] = useState<consulta[]>([]);
  const where = whereCluster();
  const { granularidad } = useGranularidad();

  const queryCluster = `SELECT cast(Cluster as char) label, sum(Certificados_deposito_termino ) valor FROM dataSetMIAD ${where} GROUP BY Cluster ORDER BY Cluster asc`;
  const queryMunicipio = `SELECT Municipio label, Certificados_deposito_termino valor FROM dataSetMIAD ${where} ORDER BY valor desc`;
  ...
  const chartData = {
    ...
    datasets: [
      {
        label: "Certificado de Deposito a Termino",
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `Certificado de Deposito a Termino por ${granularidad}`,
        font: {
          size: 24,
