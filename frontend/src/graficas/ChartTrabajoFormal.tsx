...
export function ChartTrabajoFormal() {
  const [info, setinfo] = useState<consulta[]>([]);
  const where = whereCluster();
  const { granularidad } = useGranularidad();

  const queryCluster = `SELECT cast(Cluster as char) label, avg(Porc_trabajos_formales) valor FROM dataSetMIAD ${where} GROUP BY Cluster ORDER BY Cluster asc`;
  const queryMunicipio = `SELECT Municipio label, Porc_trabajos_formales valor FROM dataSetMIAD ${where} ORDER BY valor desc`;
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `Porcentaje Trabajo Formal por ${granularidad}`,
        font: {
          size: 24,
