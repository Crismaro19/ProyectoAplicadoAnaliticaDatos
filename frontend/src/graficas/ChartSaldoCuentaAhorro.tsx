...
export function ChartSaldoCuentaAhorro() {
  const [info, setinfo] = useState<consulta[]>([]);
  const where = whereCluster();
  const { granularidad } = useGranularidad();

  const queryCluster = `SELECT cast(Cluster as char) label, sum(Total_Saldo_Cuentas_Ahorro ) valor FROM dataSetMIAD ${where} GROUP BY Cluster ORDER BY Cluster asc`;
  const queryMunicipio = `SELECT Municipio label, Total_Saldo_Cuentas_Ahorro valor FROM dataSetMIAD ${where} ORDER BY valor desc`;
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `Saldo Cuenta Ahorro por ${granularidad}`,
        font: {
          size: 24,
