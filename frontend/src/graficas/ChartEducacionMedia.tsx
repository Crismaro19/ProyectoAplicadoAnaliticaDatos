...
export function ChartEducacionMedia() {
  const [info, setinfo] = useState<consulta[]>([]);
  const where = whereCluster();
  const { granularidad } = useGranularidad();

  const queryCluster = `SELECT cast(Cluster as char) label, avg(Cobertura_edu_media) valor FROM dataSetMIAD ${where} GROUP BY Cluster ORDER BY Cluster asc`;
  const queryMunicipio = `SELECT Municipio label, Cobertura_edu_media valor FROM dataSetMIAD ${where} ORDER BY valor desc`;
  ...
  const chartData = {
    ...
    datasets: [
      {
        label: "Cobertura Educacion Media",
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `Cobertura Educacion Media por ${granularidad}`,
        font: {
          size: 24,
